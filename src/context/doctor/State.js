...
export const DoctorState = () => {
  ...
  const getDoctorsByCategoryId = async (categoryId) => {
    try {
      ...
      const response = await axios.get(`${API_URLS.Categories}/${categoryId}/doctors`);
      dispatch({type: DoctorActions.GET_DOCTORS, payload: response.data?.doctors});
  ...
  const getDoctorDetails = async (doctorId) => {
    try {
      dispatch({type: DoctorActions.SET_LOADING_DOCTOR_DETAILS, payload: true});
      const response = await axios.get(`${API_URLS.Doctors}/${doctorId}`);
      dispatch({type: DoctorActions.GET_DOCTOR_DETAILS, payload: response.data});
      dispatch({type: DoctorActions.SET_LOADING_DOCTOR_DETAILS, payload: false});
    } catch (error) {
...
// https://eashaop-af7l.onrender.com/api/reviews?doctorId=691d70e9f3c0890f1a2f7b89
  const getDoctorReviews = async (doctorId) => {
    try {
      dispatch({type: DoctorActions.SET_LOADING_DOCTOR_REVIEWS, payload: true});
      const response = await axios.get(`${API_URLS.Reviews}?doctorId=${doctorId}`);
      dispatch({type: DoctorActions.GET_DOCTOR_REVIEWS, payload: response.data});
      dispatch({type: DoctorActions.SET_LOADING_DOCTOR_REVIEWS, payload: false});
    } catch (error) {
